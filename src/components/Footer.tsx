...
export function Footer() {
  return (
    <footer className="bg-brand-950 text-neutral-200">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-14 lg:grid-cols-5">
        <div className="lg:col-span-1">
          <div className="[&_div]:text-white [&_svg_circle]:fill-transparent [&_svg_circle]:stroke-white/70">
            <Logo />
          </div>
          <p className="mt-4 text-xs leading-relaxed text-neutral-400 max-w-xs">
        ...
        <div>
          ...
          <ul className="grid grid-cols-2 gap-y-2 text-sm text-neutral-300">
            ...
            <li><Link href="#jois-cam" className="hover:text-white">Jois Cam</Link></li>
        ...
        <div>
          ...
          <div className="relative h-32 w-full overflow-hidden rounded-xl bg-neutral-200">
            <div
              className="absolute inset-0 bg-cover bg-center opacity-80"
              style={{
                backgroundImage:
                  "url('https://maps.googleapis.com/maps/api/staticmap?center=-14.0678,-75.7286&zoom=12&size=400x200&maptype=roadmap&key=placeholder')",
              }}
            />
            <div className="absolute inset-0 flex items-center justify-center">
              <MapPin className="h-8 w-8 text-brand-600 drop-shadow" />
            </div>
          </div>
        </div>
      </div>
      <div className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-5 text-center text-xs text-neutral-500">
          © 2024 Jois 0 Waste. Todos los derechos reservados.
        </div>
      </div>
    </footer>
  );
